...
import { getFamilyTaskDateClass, formatTaskDateDisplay } from './dateUtils';
...
export default function FamilyTaskRow({
  task,
  editingRowIndex,
  submitting,
  editForm,
  ...
  if (isEditing) {
    return (
      <div 
        ...
        className={`px-2.5 py-2.5 bg-zinc-50 hover:bg-zinc-100/70 flex flex-col gap-2 rounded-sm transition-colors font-sans ${isLast ? 'border-b-0' : 'border-b border-zinc-100/70'}`}
      >
        ...
        <div className="flex-1 flex flex-wrap items-center gap-1.5">
          ...
          <select
             ...
             className={`h-7 px-1.5 rounded-sm text-[13px] font-normal border border-zinc-200/80 cursor-pointer focus:ring-1 focus:ring-primary leading-none ${getFamilyClassBadgeClass(editForm.familyClass)}`}
          >
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <input
              ...
              className={`h-7 w-[125px] px-1.5 bg-white border border-neutral-200 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/20 hover:border-neutral-300 text-[13px] rounded-sm font-normal text-right transition-all leading-none ${getFamilyTaskDateClass(editForm.date)}`}
            />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-1.5 border-t border-dotted border-zinc-200 mt-0.5">
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-xs font-sans">
            ...
            <select
              ...
              className={`h-7 px-1.5 border border-zinc-200 rounded-sm text-[13px] font-normal cursor-pointer font-sans leading-none ${getCategoryBadgeClass(editForm.category)}`}
            >
  ...
  return (
    <div 
      key={task.sheetRowIndex} 
      className={`group relative pl-0.5 pr-0.5 py-1.5 bg-white hover:bg-zinc-50/75 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[13px] transition-colors rounded-none font-normal ${isLast ? 'border-b-0' : 'border-b border-zinc-100/70'}`}
    >
      ...
      <div className="w-full sm:hidden flex flex-col gap-1.5">
        <div className="flex items-start justify-between gap-1.5 w-full">
          <div className="flex-1 flex flex-wrap items-center gap-1.5 min-w-0">
            {task.familyClass && (
              <span className={`px-1.5 py-0.5 rounded text-[13px] font-normal tracking-tight ${getFamilyClassBadgeClass(task.familyClass)} shrink-0`}>
                {task.familyClass}
              </span>
            )}
        ...
        {/* Metadata starts at x=0, matching the familyClass badge */}
