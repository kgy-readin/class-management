...
export default function NoticeForm() {
  ...
  const [allTabs, setAllTabs] = useState<DocTab[]>(() => {
    const cached = localStorage.getItem('webapp_notice_form_tabs_backup');
  ...
  useEffect(() => {
    ...
    if (!pathname.startsWith('/noticeform')) return;

    const parts = pathname.split('/').filter(Boolean); // ["noticeform", "hash"]
    if (parts[1]) {
  ...
  const handleSaveEdit = async () => {
    ...
    localStorage.setItem('webapp_notice_form_tabs_backup', JSON.stringify(updatedTabs));
    setIsEditing(false);

    try {
      setSavingTab(true);
      await noteApi.saveTabSpecification(selectedTabId, editText);
      toast.success(MESSAGES.noticeform.saveSuccess);
    } catch (error: any) {
      console.error('GAS saveTabSpecification Failed:', error);
      toast.error(MESSAGES.noticeform.saveError(error.message), {
        duration: 6000
      });
    } finally {
      setSavingTab(false);
                      ...
                      <div className="pl-4 border-l border-neutral-100 ml-4.5 space-y-0.5 animate-in fade-in slide-in-from-top-1 duration-200">
                        {folder.childTabs.map((child) => {
                          ...
                          return (
                            <div
                              key={child.id}
                              onClick={() => navigate(`/noticeform/${getShortHash(child.id)}`)}
